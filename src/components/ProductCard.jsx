...
export default function ProductCard({ product, onAddToCart }) {
  ...
  const [slideDir, setSlideDir] = useState("right"); // Thêm state hướng
  ...
  const prevImg = (e) => {
    e.stopPropagation();
    setSlideDir("left"); // Đặt hướng
    setImgIdx((idx) => (idx === 0 ? images.length - 1 : idx - 1));
  };
  const nextImg = (e) => {
    e.stopPropagation();
    setSlideDir("right"); // Đặt hướng
    setImgIdx((idx) => (idx === images.length - 1 ? 0 : idx + 1));
  };
  ...
  return (
    ...
    >
      ...
      {/* Image slider */}
      <div className="relative z-10 w-full h-[60%] flex items-center justify-center rounded-t-lg">
        <img
          src={images[imgIdx]}
          alt={product.name}
      ...
      {/* Product info */}
      <div className="relative z-10 flex flex-col gap-2 px-3 py-2 bg-black/60 rounded-b-lg">
